...
namespace Rock.ElectronicSignature
{
    ...
    public static class ElectronicSignatureHelper
    {
        ...
        {
            var includedMergedFields = new Dictionary<string, object>( mergeFields );

                // if there isn't a SignatureInformation merge field in the template, add one at the bottom.
                var hasSignatureInformationLavaMergeField = new System.Text.RegularExpressions.Regex( @"(?<=\{).+" + ElectronicSignatureHelper.SignatureInformationMergeKey + @".+(?<=\})" );
                if ( !hasSignatureInformationLavaMergeField.IsMatch( lavaTemplate ) )
                {
                    lavaTemplate += "<br>{{ " + ElectronicSignatureHelper.SignatureInformationMergeKey + " }}";
                }

            return lavaTemplate.ResolveMergeFields( includedMergedFields );
        }
